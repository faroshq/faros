...
//     member ID ("faros:static:<hash>") waits for that many hash characters,
...
function staticHash(q: string): string | null {
  const rest = q.startsWith('faros:') ? q.slice('faros:'.length) : q
...
function cached(q: string): UserSuggestion[] | null {
  ...
  for (let n = q.length - 1; n >= USER_SEARCH_MIN_QUERY; n--) {
    ...
    // would have included the same kinds of account: "faros" never returns
    // static-token accounts, "faros:static:02d4b" does.
