...
# edges.railgrid.ai — KubernetesCluster (type=kubernetes) and LinuxServer
# (type=server) — instead of the old single Edge with spec.type. The workspace
# must have the edges provider ENABLED (APIBinding to edges.providers.railgrid.ai)
...
# Map the type to its kind + resource in group edges.railgrid.ai.
...
# Extract the cluster path from URL like https://console.127.0.0.1.sslip.io:9443/clusters/abc123
CLUSTER_PATH=""
...
elapsed=0
while true; do
  JOIN_TOKEN=$(kubectl get "${EDGE_RESOURCE}.edges.railgrid.ai" "${EDGE_NAME}" -o jsonpath='{.status.joinToken}' 2>/dev/null || echo "")
...
RAILGRID_EDGE_NAME=${EDGE_NAME}
RAILGRID_EDGE_TYPE=${EDGE_TYPE}
RAILGRID_EDGE_LABELS=${LABELS}
RAILGRID_EDGE_CLUSTER=${CLUSTER_PATH}
RAILGRID_EDGE_JOIN_TOKEN=${JOIN_TOKEN}
EOF
