...
| `skills list\|install [skill…]` | `--repo owner/name` (faroshq/faros), `--ref` (main), `--target claude\|codex\|all`, `--scope user\|project`, `--dir`, `--force`, `-o json\|yaml` | Downloads the repo's source tarball from GitHub at that moment and installs every `skills/<name>/` (or the named ones) to `~/.claude/skills` and `~/.agents/skills` (project scope: `./.claude/skills`, `./.agents/skills`). Writes `.faros-skill.json` (repo, ref, commit, files) and only replaces directories carrying it; `is a symlink`, `was not installed by 'faros skills'; pass --force`, `has no branch, tag or commit "<ref>"`. No hub access needed. |
