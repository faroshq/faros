...
| `unknown command "proxy" for "faros mcp"` (or the client's `faros` MCP server exits at once) | The installed CLI predates `faros mcp proxy` | Reinstall the CLI (`curl -fsSL https://downloads.faros.sh/install.sh \| sh`), then reconnect the client |
