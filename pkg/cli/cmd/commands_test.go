...
func TestWhoamiShowsMemberID(t *testing.T) {
	...
	hub.handle("GET /api/users/me", func(w http.ResponseWriter, r *http.Request) {
		writeTestJSON(w, map[string]any{
			...
			"displayName":  "faros:static:47b9dce0e91570a1",
	...
	for _, want := range []string{
		"User:       faros:static:47b9dce0e91570a1",
		"Member ID:  faros:static:47b9dce0e91570a1 (give this to an admin who wants to add you)",
	} {
		if !strings.Contains(out, want) {
	...
	var v whoamiView
	if err := json.Unmarshal([]byte(mustRun(t, path, "whoami", "-o", "json")), &v); err != nil || v.MemberID != "faros:static:47b9dce0e91570a1" {
