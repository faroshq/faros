...
import (
	...
	"github.com/faroshq/faros/pkg/util/cmp"
)
...
func New(log *zap.Logger, restconfig *rest.Config) (DynamicHelper, error) {
	dh := &dynamicHelper{
		log:        log,
...
func (dh *dynamicHelper) findGVR(groupKind, optionalVersion string) (*schema.GroupVersionResource, error) {
	...
	if len(matches) == 0 {
		return nil, fmt.Errorf("The groupKind %s was not found.", groupKind)
	}
	...
	if len(matches) > 1 {
		...
		return nil, fmt.Errorf("The groupKind '%s' matched multiple groupKinds (%s).", groupKind, strings.Join(matchesGK, ", "))
	}
...
func (dh *dynamicHelper) Ensure(ctx context.Context, o *unstructured.Unstructured) error {
	...
	return retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		if errors.IsNotFound(err) {
			dh.log.Sugar().Infof("Create %s", keyFuncO(o))
			_, err = dh.dyn.Resource(*gvr).Namespace(o.GetNamespace()).Create(ctx, o, metav1.CreateOptions{})
			return err
		}
		if err != nil {
		...
		dh.log.Sugar().Infof("Update %s: %s", keyFuncO(o), diff)
