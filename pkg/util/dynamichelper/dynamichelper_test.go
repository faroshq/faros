...
func TestFindGVR(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantErr: fmt.Errorf("The groupKind %s was not found.", "baremetalhost/status"),
		},
		{
			name:    "empty resources",
			kind:    "configmap",
			wantErr: fmt.Errorf("The groupKind %s was not found.", "configmap"),
		},
		{
			name: "find all kinds",
			resources: []*metav1.APIResourceList{
			...
			kind:    "baremetalhost",
			want:    nil,
			wantErr: fmt.Errorf("The groupKind '%s' matched multiple groupKinds (baremetalhost.metal3.io, baremetalhost.plastic.io).", "baremetalhost"),
		},
	}
...
func TestEnsure(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := dh.Ensure(context.TODO(), tt.new)
