...
// root:railgrid:providers:<name>, a "provider" ServiceAccount with cluster-admin
// inside that workspace, and a long-lived kubeconfig the provider pod mounts —
// written into a Secret in root:railgrid:system:providers (the same workspace this CR
// lives in). It also binds the providers.railgrid.ai APIExport (CatalogEntry)
...
// minted kubeconfig).
//
// Provider lives in the admin.railgrid.ai APIExport, bound ONLY in
// root:railgrid:system:providers — a provider cannot create a Provider from its own
...
// (root:railgrid:providers:<name>) and the ServiceAccount, so the spec carries
// only optional knobs.
type ProviderSpec struct {
	// DisplayName is informational, shown in admin tooling. The provisioned
	// sub-workspace is always root:railgrid:providers:<metadata.name> regardless.
	// +optional
	// +kubebuilder:validation:MaxLength=128
	DisplayName string `json:"displayName,omitempty"`

	// SecretName overrides the name of the kubeconfig Secret the controller
	// writes into root:railgrid:system:providers. Defaults to "<name>-kubeconfig".
	// +optional
...
type ProviderSecretRef struct {
	...
	// root:railgrid:system:providers. Typically "default".
	// +optional
