...
| `deployments` | GraphQL gateway for RepositorySync reads; its backend serves the embedded portal and health endpoints | ✅ Conforms |
...
| `deployments` | init/serve split: minted provider SA + APIExportEndpointSlice multicluster manager; required Code checkout claim plus explicit optional target-resource claims | ✅ Conforms |
...
| `deployments` → target APIs | Applies unstructured desired-state objects through bound, explicitly authorized APIExport permission claims; no target-provider backend URL, types, or runtime-cluster credential | ✅ Conforms |
