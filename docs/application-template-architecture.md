...
`cfgate-system`), configured via `${faros.gatewayName}` / `${faros.gatewayNamespace}`
(`FAROS_GATEWAY_NAME` / `FAROS_GATEWAY_NAMESPACE`). Where this document says
`Ingress` / `ingressClassName` / `${faros.ingressClass}` below, read HTTPRoute /
`parentRefs` / `${faros.gatewayName}`. The current operational reference is
...
runtime cluster** in a per-tenant namespace (`faros-tenants-<hash>`), and the only templates
...
(`infrastructure.faros.sh`) that materializes a 3-tier app plus two net-new layers:
...
- An **auth layer** — a **per-app oauth2-proxy** in front of the frontend, supporting two
  OIDC modes: **Platform SSO** (the faros identity, via the platform Dex) and **BYO external
...
2. **Exposure follows faros's dial-out philosophy.** Cloudflare Tunnel opens an *outbound*
   tunnel from the runtime cluster — no inbound load balancer, no public ingress IP, no
   firewall holes — the same reverse-tunnel shape faros already relies on.
...
provider workspace (root:faros:providers:infrastructure)
  ...
  │    schemas: + application.infrastructure.faros.sh
  ...
  └─ (platform config, not tenant-visible)
       FAROS_APP_BASE_DOMAIN   — Cloudflare zone apps live under
       FAROS_INGRESS_CLASS     — exposure-layer ingressClassName (default "cloudflare")
       FAROS_OIDC_ISSUER_URL   — platform Dex issuer (Platform SSO mode)
       FAROS_DEX_GRPC_ADDR     — Dex client-management API (Platform SSO mode)
...
runtime cluster (shared kro cluster)            per-tenant namespace faros-tenants-<hash>
  │
...
status:
  ...
  dbConnectionSecretRef:
    ...
    namespace: faros-tenants-9f2db7ed8014
```
...
| `FAROS_APP_BASE_DOMAIN` | Cloudflare zone apps live under |
| `FAROS_INGRESS_CLASS` | exposure-layer `ingressClassName` (default `cloudflare`) |
| `FAROS_INGRESS_ANNOTATIONS` | optional class-specific Ingress annotations |
| `FAROS_OIDC_ISSUER_URL` | platform Dex issuer (Platform SSO mode) |
| `FAROS_DEX_GRPC_ADDR` (+ CA/cert) | Dex client-management API (Platform SSO mode) |

The seed template is static YAML, so these can't be baked in. The kro backend gains a small
**faros-level token-substitution pass** (reserved tokens `${faros.appBaseDomain}`,
`${faros.ingressClass}`, ingress annotations) applied to `spec.backendConfig` before
`buildRGD`. These are distinct from kro's own `${...}` references and are documented as a
reserved set.

## 6. The kro resource graph (`spec.backendConfig`)

All children set `namespace: default`, remapped to `faros-tenants-<hash>` on the runtime
...
- **Ingress** (plain `networking.k8s.io/v1`, controller-agnostic): host `${…expose.fqdn}`,
  `spec.ingressClassName: ${faros.ingressClass}`, optional annotations, single path `/` →
...
The faros **hub is an OIDC relying party, not a provider** (`pkg/server/auth/handler.go`
builds `oidc.NewProvider(IssuerURL)` against an upstream; it is a PKCE public client
configured with `--idp-issuer-url`/`--idp-client-id` in `cmd/faros-hub/main.go`). The actual
...
The intended "guard with faros's own identity" mode. Because the hub is a relying party (not
...
faros Dex deployment has **no gRPC client-management API enabled** and uses **ephemeral
...
Not faros code — installed via Helm/GitOps on the shared runtime cluster:
...
helper, the `${faros.ingressClass}` substitution in `backend/kro`, the `application` template,
...
|---|---|---|
| **AP-1** | Exposure layer (Cloudflare Tunnel) — *ops, follow-up* | cloudflared + cloudflare-tunnel ingress controller on the runtime cluster against a Cloudflare zone; a test Ingress (cloudflare class) auto-creates the DNS record + tunnel route and serves `https://x.apps.<base>` with edge TLS; a `runtime-ingress.md` covers setup + how to swap the class. (Ops/doc, not faros code.) |
| **AP-2** | Config + host computation + controller stamping — *landed* | `FAROS_APP_BASE_DOMAIN`/`FAROS_INGRESS_CLASS` plumbed; the controller injects `spec.expose.fqdn` + `spec.credentialsSecretName`; host formula + DNS-label prefix validation; token-substitution pass in `backend/kro`. Unit tests for the formula + substitution. |
| **AP-3** | `Application` Template seed — *landed* | `install/templates/application.yaml` added; applying it establishes `applications.infrastructure.faros.sh`, lists it in `APIExport.spec.schemas`, and kro accepts the RGD; `kubectl get templates` shows it. |
...
| **AP-5** | Platform SSO mode (Dex client minting) — *deferred (needs Dex infra, §7.2)* | hub Dex gRPC + persistent storage + Service + TLS (separate epic); provider `dex/` gRPC helper; `FAROS_OIDC_ISSUER_URL` + `FAROS_DEX_GRPC_ADDR` plumbed; the controller mints a Dex client + injects issuer/clientID/secret; teardown deletes it; orphan sweeper for the failure window. |
...
- **R5 — Platform SSO depends on Dex gRPC + client lifecycle.** Requires Dex deployed with the
  ...
  SQL) — an in-memory dev Dex drops minted clients on restart. Confirm the faros Dex
...
- **R6 — "Platform SSO" ≠ hub issuer.** The hub is a relying party; platform SSO points at
  Dex. If faros later federates directly to a hosted IdP with no Dex, platform mode needs that
