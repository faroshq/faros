...
| [Helm](https://helm.sh/docs/intro/install/) 3 | Only needed to connect extra clusters by hand; `faros dev init` installs the hub, provider and agent charts itself |
...
This creates one kind cluster, `faros-hub`, and installs three things into it from the published Helm charts:
...
- the faros agent, which registers the kind cluster itself as a Kubernetes edge named `local` in your default workspace (`--with-edge=false` skips this, `--edge-name` renames it).

Use `--chart-path deploy/charts/faros-hub --provider-chart-repo .` to run the charts from a checkout. `--worker-count N` adds plain kind clusters for connecting more edges by hand, as in section 3b.
...
faros edge kubeconfig local > local.yaml
...
```bash
kubectl --kubeconfig faros-hub.kubeconfig -n faros-agent logs deploy/faros-agent
```

### Step 3b: Connect another cluster by hand

To see the registration flow itself, or to connect a second cluster, create the environment with `--worker-count 1` so a plain `faros-agent` kind cluster exists on the same Docker network, then register an edge and hand its credentials to an agent there:

```bash
faros edge create worker --labels env=dev

# the hub mints a kubeconfig for the edge; extract it
kubectl get secret -n faros-system edge-worker-kubeconfig \
...
```bash
faros mcp claude --ca-file faros-hub-ca.crt   # Claude Code
faros mcp codex --ca-file faros-hub-ca.crt    # Codex
```

Each command adds the server with the workspace's long-lived MCP token, then prints how to start the client. The local hub's certificate is signed by the dev CA that `faros dev init` wrote to `faros-hub-ca.crt`, so the client has to trust it:

```bash
NODE_EXTRA_CA_CERTS=$PWD/faros-hub-ca.crt claude
```

Codex cannot skip certificate verification. `faros mcp codex` writes a bundle of your system roots plus the dev CA and prints the matching `CODEX_CA_CERTIFICATE=… codex` line, which needs Codex 0.129.0 or later. Without `--ca-file`, `faros mcp claude` falls back to `NODE_TLS_REJECT_UNAUTHORIZED=0`, which turns verification off for everything that session connects to.

For another client, `faros mcp url --mcpserver-name default` prints the endpoint and configuration snippets. Then ask the assistant to list nodes on `local`. Tools from every enabled provider appear on the same endpoint, and every call runs under your identity and RBAC.

## Step 5: Enable a provider

Providers are Helm releases that register with the hub. `faros dev init` already installed edges, infrastructure, code, agents and App Studio and enabled them in your default workspace; edges is what made `local` possible. The [quickstart provider](https://github.com/faroshq/faros/tree/main/providers/quickstart) is the smallest one and a good next install: add `quickstart` to `--providers` and re-run `faros dev init`, and the [developer guide]({% link developers.md %}) covers writing your own.

Apps that infrastructure templates publish are served at `https://<app>.apps.127.0.0.1.sslip.io:10443` through an Envoy Gateway in the cluster. They share a site with the portal, so private-app sign-in works in App Studio's preview, and the certificate is self-signed like the hub's. App Studio's signed preview bridge is off. Model credentials for agents and App Studio are created in the portal under Models, per workspace. The code provider signs in to GitHub only when `GITHUB_OAUTH_CLIENT_ID` and `GITHUB_OAUTH_CLIENT_SECRET` are set for `faros dev init`; register `https://console.127.0.0.1.sslip.io:9443/services/providers/code/oauth/github/callback` as the OAuth App's callback. Without them, add a GitHub token as a Connection in the portal. Once a provider is registered, enable it for a workspace in the portal at `https://console.127.0.0.1.sslip.io:9443/ui` and its APIs and tools appear.
...
```bash
kubectl --kubeconfig faros-hub.kubeconfig -n faros-agent logs deploy/faros-agent
kubectl --kubeconfig faros-hub.kubeconfig -n faros-providers logs deploy/edges
```
...
### `faros dev init` fails while onboarding a provider

The provider automation signs in to the hub with `dev-token` and calls the admin API, so it needs token login; `--with-dex` disables that and skips providers and the edge. A timeout waiting for a provider kubeconfig or a join token usually means the hub or provider pod is not healthy: `kubectl --kubeconfig faros-hub.kubeconfig get pods -A` shows which.
