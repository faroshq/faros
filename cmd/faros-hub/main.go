...
// Package main is the entrypoint for the faros-hub server.
package main
...
func main() {
	...
	cmd := &cobra.Command{
		Use:   "faros-hub",
		Short: "Faros hub server - multi-tenant control plane",
	...
	cmd.Flags().StringVar(&opts.PublishedAppsDomain, "published-apps-domain", "", "DNS zone published apps are served under (e.g. apps.faros.example); enables the login-time published-app authorize endpoints")
	...
	cmd.Flags().StringVar(&opts.IDPClientID, "idp-client-id", "faros", "OIDC identity provider client ID")
	...
	cmd.Flags().StringVar(&opts.HubInternalURL, "hub-internal-url", "", "Address in-cluster components use to reach this hub instead of --hub-external-url; baked into minted provider kubeconfigs (default: --hub-external-url). Set to the hub's in-cluster Service so provider→hub traffic never leaves the cluster, e.g. https://faros-faros-hub.faros.svc.cluster.local:9443.")
