...
test('calls host fetch with tenant scope and preserves stable command identity on uncertainty', async () => {
 const calls = []; const api = new API({ tenant: 'one', fetch: async (path, init) => { calls.push({ path, init }); throw new Error('connection lost') } }, 'default', new AbortController().signal)
...
test('discard stale workspace responses even if the host transport ignores abort', async () => {
 const controller = new AbortController(); const api = new API({ tenant: 'one', fetch: async () => { controller.abort(); return new Response(JSON.stringify({ items: [{ metadata: { name: 'private' } }] })) } }, 'default', controller.signal)
