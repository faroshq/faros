...
function load() { void read.run(async api => { const item = await api.get('teams', props.name); const conn = await api.get('connections', String(item.spec?.connection)); return { item, conn }; }, result => {
...
<template>
  <ResourcePage :title="team?.status?.name || name" kind="Team" :loaded="read.state.loaded" :loading="read.state.loading" :error="read.state.error" retryable @retry="load">
    ...
    <template #actions><button class="k-btn k-btn--ghost" :disabled="read.state.loading || deletion.state.loading" @click="load">Refresh</button><ActionMenu v-if="team" label="More team actions" :disabled="deletion.state.loading || !!team.metadata.deletionTimestamp" :items="[{ id: 'remove', label: 'Remove from Faros', tone: 'danger' }]" @select="deletion.remove(team)" /></template>
    <div v-if="team" class="linear-detail-content">
      <TaskFeedback :task="deletion.state" />
      <ResourceSectionCard title="Details"><dl class="linear-facts"><div><dt>Connection</dt><dd><a :href="session.href(resourcePath('connections', String(team.spec?.connection)))" @click.prevent="session.navigate(resourcePath('connections', String(team.spec?.connection)))">{{ team.spec?.connection }}</a></dd></div><div><dt>Team key</dt><dd>{{ team.status?.key || '—' }}</dd></div><div><dt>Linear team ID</dt><dd>{{ team.spec?.teamID }}</dd></div><div><dt>Message</dt><dd>{{ team.status?.message || '—' }}</dd></div></dl></ResourceSectionCard>
      <IssuesView v-if="!team.metadata.deletionTimestamp && team.spec?.connectionUID === connection?.metadata.uid" :scope="{ connection: String(team.spec?.connection), team: String(team.spec?.teamID) }" />
