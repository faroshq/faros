...
<template>
  <section class="linear-page">
    <header class="linear-page-head"><div><h2 class="linear-page-title">Connections</h2><p class="linear-page-meta">Connect a Linear workspace using a credential stored in this namespace.</p></div><div class="linear-actions"><button class="k-btn k-btn--ghost" :disabled="read.state.loading" @click="load">Refresh</button><button class="k-btn k-btn--primary" @click="session.navigate('create/connection')">Add connection</button></div></header>
    <FirstRunGuide v-if="read.state.loaded && !read.state.error && !resources.length" :steps="[]" title="Connect Linear" description="Reference an existing credential to browse teams and manage issues." primary-label="Add connection" @primary="session.navigate('create/connection')" />
