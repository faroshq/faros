...
func setup(t *testing.T) (Engine, *unstructured.Unstructured) {
	...
	op := actionapi.Receipt{TypeMeta: metav1.TypeMeta{APIVersion: "linear.internal.faros.sh/v1alpha1", Kind: "ActionReceipt"}, ObjectMeta: metav1.ObjectMeta{Name: "op-1", UID: "op-one", Annotations: map[string]string{"linear.internal.faros.sh/connection-uid": "connection-one"}}, Spec: actionapi.Input{Connection: "linear", Action: "createIssue", TeamID: "allowed", Title: &title}}
