...
`name@sha256:<64 lowercase hex digits>` reference. The shipped image recipe is
...
must provide the immutable universal image reference.
...
| `FAROS_CODING_SANDBOX_ENABLED` | `false` | Opts into seeding/admitting the platform-owned universal coding sandbox; enabled deployments require an immutable universal image |
