...
// GraphQL-backed tenant access (ported from the agents provider). vibe-studio
// reaches a tenant's kcp workspace through the hub's embedded GraphQL gateway
// at <hubBase>/graphql/<clusterID>, authenticating as the caller. Unlike the
// hub's kcp user-proxy (which gates every request to the caller's
// DefaultCluster), the gateway serves any workspace the caller has RBAC in,
// so vibe-studio works in non-default workspaces.
//
