...
func TestCatalogEntryContractStaysInSync(t *testing.T) {
	...
	infraHash, codeHash := "catalog-parity-infrastructure", "catalog-parity-code"
	initialized := catalogParityInitContract(t, vibeStudioPermissionClaims(infraHash, codeHash), map[string]string{
		infraHash: "Provider/infrastructure",
		codeHash:  "Provider/code",
	})
...
func catalogParityRenderEntry(t *testing.T) catalogParityContract {
	t.Helper()
	raw, err := exec.Command("helm", "template", "catalog-parity", "deploy/chart", "--set", "catalogEntry.enabled=true", "--set", "catalogEntry.renderAsConfigMap=true").CombinedOutput()
	if err != nil {
...
func catalogParityDecodeEntry(t *testing.T, source string, raw []byte) catalogParityContract {
	...
	contract := catalogParityContract{Dependencies: append([]catalogParityDependency(nil), entry.Spec.Dependencies...), RequiredResources: append([]catalogParityResource(nil), entry.Spec.APIExport.RequiredResources...)}
	for _, claim := range entry.Spec.APIExport.PermissionClaims {
		...
		contract.PermissionClaims = append(contract.PermissionClaims, catalogParityClaim{Group: claim.Group, Resource: claim.Resource, Verbs: append([]string(nil), claim.Verbs...), IdentitySource: identitySource})
	}
