...
<template>
  <div :class="createRoute ? 'agents-menu agents-create-page k-create-page' : 'agents-panel k-card agents-route-panel'">
    ...
    <template v-else>
      ...
      <div v-if="catalogError && !catalogHasSnapshot" class="k-error" role="alert">Model catalog unavailable: {{ catalogError }} <button class="k-btn k-btn--ghost" :disabled="catalogLoading" @click="loadCatalog">{{ catalogLoading ? 'Retrying…' : 'Retry' }}</button></div>
      <div v-else-if="catalogError" class="k-stale" role="status">Could not refresh the model catalog. Showing the last loaded catalog. {{ catalogError }} <button class="k-btn k-btn--ghost" :disabled="catalogLoading" @click="loadCatalog">{{ catalogLoading ? 'Retrying…' : 'Retry' }}</button></div>
