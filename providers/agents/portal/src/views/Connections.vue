  ...
  <div v-else class="agents-menu">
    <div class="agents-panel k-card agents-route-panel">
      ...
      <ResourceTable v-else :columns="[{ key: 'name', label: 'Name', primary: true }, { key: 'kind', label: 'Kind' }, { key: 'type', label: 'Type' }, { key: 'endpoint', label: 'Endpoint / channel' }, { key: 'actions', label: '', ariaLabel: 'Actions' }]" :rows="rows" row-key="id" aria-label="Connections" :loaded="connections.hasSnapshot" :loading="connections.loading" :error="connections.error" :stale="connections.hasSnapshot && !!connections.error" retryable searchable search-placeholder="Search connections…" :search-keys="['name', 'type', 'endpoint']" :filters="filters" paginated :interactive="false" @retry="store.load('connections')">
        ...
        <template #endpoint="{ row }"><span class="agents-resource-endpoint" :title="endpoint(asConnection(row))">{{ endpoint(asConnection(row)) || '—' }}</span><span v-if="needsInstance(asConnection(row))" class="k-badge agents-badge k-badge--warning agents-badge-warn" title="Edit this connection and name the searxng instance it should search through">needs an instance</span><span v-if="wiring(asConnection(row)) === 'unwired'" class="k-badge agents-badge k-badge--warning agents-badge-warn" title="No agent has been granted this tool — add it under an agent's Config → Tools">not wired to an agent</span><span v-else-if="wiring(asConnection(row)) === 'unknown'" class="k-badge agents-badge k-badge--muted" title="Agent or toolset assignments are unavailable">wiring unknown</span></template>
