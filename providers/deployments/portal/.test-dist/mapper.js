...
export function mapRelease(value) {
    ...
    if (!Array.isArray(rawArtifacts))
        throw new Error('release.spec.artifacts must be an array');
...
export function mapDeployment(value, releases = new Map()) {
    ...
    if (!Array.isArray(rawConditions))
        throw new Error('deployment.status.conditions must be an array');
...
export function mapResourceList(releasesValue, deploymentsValue) {
...
function condition(snapshot, type) {
    return snapshot.conditions.find(item => item.type.toLowerCase() === type.toLowerCase());
}
export function isCurrentEvidence(snapshot, item) {
...
export function conditionIsTrue(snapshot, type) {
...
export function evidenceState(snapshot) {
    if (snapshot.deletionTimestamp)
        return 'deleting';
    if (snapshot.phase?.toLowerCase() === 'invalid')
        return 'invalid';
    if (snapshot.phase?.toLowerCase() === 'ready' && conditionIsTrue(snapshot, 'Ready'))
        return 'ready';
    if (conditionIsTrue(snapshot, 'Applied'))
        return 'applied';
    if (snapshot.phase?.toLowerCase() === 'pending' || snapshot.conditions.length > 0)
        return 'pending';
    return 'unknown';
}
