...
import { buildElements, mountGraph, RELATION_DIR, RELATION_LABELS, themeStyle, type GraphHandle } from '../graph'
...
function spec(): QuerySpec {
  ...
  const names = ['descendants+', 'references', 'selects', 'selected-by', 'owners', 'linked+', 'grouped', 'namespace', 'namespaced']
  return { maxDepth: 5, cluster: props.anchor.cluster ? { name: props.anchor.cluster } : undefined, filter: { objects: [filter] }, objects: { id: true, cluster: true, relations: Object.fromEntries(names.map(name => [name, name === 'namespaced' ? { limit: 200 } : {}])) } }
}
...
async function mount(): Promise<void> {
  ...
  try {
    const handle = await mountGraph(graphHost.value, built.elements, themeStyle(graphHost.value), id => { const row = built.nodeIndex[id]; if (row && id !== result.value?.id) emit('inspect', row) }, `${(context.value?.basePath || '').replace(/\/?$/, '/')}cytoscape.min.js`)
    if (current !== generation) return handle.destroy()
    graph = handle; requestAnimationFrame(() => handle.fit())
...
onBeforeUnmount(() => { controller?.abort(); destroyGraph() })
</script>
    ...
    <ResourcePage :title="title" :kind="anchor.object?.kind" :subtitle="subtitle" :loaded="loaded" :loading="loading" :error="error" :stale="loaded && !!error" retryable @retry="load">
      ...
      <template #body>
        ...
        <div v-if="representation === 'graph'">
          ...
          <div ref="graphHost" class="kuery-graph" role="region" :aria-label="`Impact graph for ${title}`" tabindex="0" />
        </div>
