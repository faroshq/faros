...
const (
	...
	projectToolListDevelopmentServices        = "list_development_services"
	projectToolUpsertDevelopmentService       = "upsert_development_service"
	projectToolDeleteDevelopmentService       = "delete_development_service"
	projectToolRestartDevelopmentService      = "restart_development_service"
	projectToolSetPrimaryPreviewService       = "set_primary_preview_service"
	projectToolListDetectedListeners          = "list_detected_development_listeners"
	projectToolUpsertProjectComponent         = "upsert_project_component"
	projectToolDeleteProjectComponent         = "delete_project_component"
	projectToolListDependencyTemplates        = "list_dependency_templates"
	projectToolListProjectDependencies        = "list_project_dependencies"
	projectToolUpsertProjectDependency        = "upsert_project_dependency"
	projectToolDeleteProjectDependency        = "delete_project_dependency"
	projectToolGetProjectBuildConfiguration   = "get_project_build_configuration"
	projectToolSetProjectBuildWorkflow        = "set_project_build_workflow"
...
func projectSystemPromptForMode(p *aiv1alpha1.Project, repository *ProjectRepositoryView, collaborationMode projectAssistantCollaborationMode, initialBuild bool) string {
	...
	if p.Spec.Template != nil && strings.TrimSpace(p.Spec.Template.Name) != "" {
		...
		b.WriteString("- Hosted development/preview template: NONE. When the authoritative turn snapshot contains a ready codingEnvironment, continue authorized source work there and persist changes to the Project workspace; lack of a hosted template is not an authoring, compiler, or test blocker. It is also not a development-process or browser-preview blocker. ")
		b.WriteString("For a template-less Project, a Project-local DevelopmentService is the long-lived development process and preview contract inside the universal sandbox. When the user's requested app needs a server or browser preview and its safe argv command and port are known, configure a private DevelopmentService with upsert_development_service, select it as primary when appropriate, wait for its observed process/port/route readiness, then obtain and inspect or interact with its preview URL before making UI-behavior claims. ")
		b.WriteString("Never infer or auto-expose a detected listener: listener observations are suggestions only, and public visibility still requires the user's explicit confirmation. Bind a hosted development template only when the user needs that template's distinct environment contract, not merely to obtain a preview. ")
		b.WriteString("Template selection is independent of repository provisioning, and repository state gates Git commits only. Inspection-only requests and Plan mode do not authorize a DevelopmentService mutation or binding a template.\n")
	}
