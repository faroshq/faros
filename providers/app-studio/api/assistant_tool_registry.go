	...
	tools := []projectAssistantTool{
		...
		projectAssistantToolFunc{
			spec: projectAssistantToolSpec{
				...
				Description: "Apply one atomic contextual patch to project-relative UTF-8 files. The patch must start with '*** Begin Patch' and end with '*** End Patch'. Start sections with '*** Add File: <path>', '*** Update File: <path>', or '*** Delete File: <path>'. A move is an Update File section with '*** Move to: <new path>' immediately below it. " + projectAssistantContextualPatchFormatInstruction + "Hunk lines start with space (context), '-' (remove), or '+' (add). Include at least three stable surrounding context lines when needed to make every match unique. Independently matchable hunks are normalized into source order, but emitting them in source order gives the clearest failures for truly stale context. Parent directories are created automatically. A multi-file patch is fully preflighted before any mutation.",
				Parameters:  json.RawMessage(fmt.Sprintf(`{"type":"object","properties":{"patch":{"type":"string","minLength":1,"maxLength":%d,"description":"Contextual patch envelope using only project-relative paths. For a move, use Update File for the old path followed immediately by Move to for the new path; Move to cannot stand alone. Hunk headers are exactly '@@' or '@@ <literal source line>'; numeric unified-diff coordinates are forbidden."}},"required":["patch"],"additionalProperties":false}`, workspace.MaxUnifiedPatchBytes)),
