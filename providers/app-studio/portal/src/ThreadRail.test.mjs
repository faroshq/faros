...
test('defaults anchored, exposes toggle and hover preview controls, and retains hover expansion when collapsed', async () => {
  ...
  assert.match(source, /function togglePanel\(\)/)
  assert.match(source, /else open\(false\)[\s\S]*toggleAnchored\(\)/)
  ...
  assert.match(source, /defineExpose\(\{[\s\S]*openAndFocus[\s\S]*toggle: togglePanel[\s\S]*previewEnter[\s\S]*previewLeave/)
  assert.match(source, /window\.matchMedia\('\(max-width: 767px\)'\)\.matches[\s\S]*mobileOpen\.value = true/)
...
test('provides row hover actions and an accessible right-click action menu', async () => {
  ...
  assert.match(source, /class="pointer-events-none absolute inset-y-0 right-0 z-20 flex items-center gap-0\.5 pl-8 pr-1 opacity-0 transition group-hover:pointer-events-auto group-hover:opacity-100 group-focus-within:pointer-events-auto group-focus-within:opacity-100"/)
