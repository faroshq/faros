...
test('keeps New tab in the horizontal tab flow while preserving tablist semantics', () => {
  ...
  const scrollStart = header.indexOf('<div class="w-fit max-w-full min-w-0 flex-initial overflow-x-auto">', groupStart)
  const tablistStart = header.indexOf('role="tablist"')
  const scrollerEnd = header.indexOf('          </div>\n          <button', tablistStart)
  const plusStart = header.indexOf('aria-label="New tab"')
  const plusButtonStart = header.lastIndexOf('<button', plusStart)
  const providerStart = header.indexOf('title="Open full provider"', plusStart)
  assert.ok(groupStart >= 0 && scrollStart > groupStart && tablistStart > scrollStart && scrollerEnd > tablistStart)
  assert.ok(plusButtonStart > scrollerEnd && providerStart > plusStart)
  assert.match(header.slice(groupStart, plusButtonStart), /flex min-w-0 flex-1 items-center gap-1/)
  assert.match(header.slice(scrollStart, scrollerEnd), /w-fit max-w-full min-w-0 flex-initial overflow-x-auto/)
  assert.match(header.slice(scrollerEnd), /^          <\/div>\n          <button/)
  const tablist = header.slice(tablistStart, scrollerEnd)
