...
// Package install holds the one-shot bootstrap steps the secrets provider
// runs against its own kcp workspace. Today that is a single step: ensuring
// an APIExportEndpointSlice exists for the provider's APIExport so the
// multicluster apiexport provider can discover tenant workspaces.
//
// The hub provisioner creates the sub-workspace, the APIResourceSchemas, the
// APIExport, the ServiceAccount, and the minted kubeconfig — but it does NOT
// create an APIExportEndpointSlice (that is the consumer's job, since the
// slice's export path and name are consumer-chosen). Without the slice,
// apiexport.New has no endpoints to watch and the controllers never engage
// any tenant cluster. EnsureAPIExportEndpointSlice closes that gap; it is
// called idempotently at serve startup (and by the init subcommand).
package install
...
const (
	...
	APIExportName = "secrets.providers.faros.sh"
	...
	// APIExport name.
	APIExportEndpointSliceName = "secrets.providers.faros.sh"
)
...
// logical-cluster path the APIExport lives in (root:faros:providers:secrets) —
