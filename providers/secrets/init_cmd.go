...
func runInitCmd(ctx context.Context) error {
	...
	log.Printf("secrets-provider init: workspace bootstrapped (export=%s path=%s schemas=%s catalogEntry=%s)", install.APIExportName, workspacePath, schemasDir, catalogEntryFile)
	return nil
}
...
//
// The secrets claim is a built-in k8s type (empty group), so it needs no
// identityHash. The controllers read each SecretStore's credential Secret AND
// write/delete the SyncedSecret projections in the tenant workspace, hence
// the write verbs — this provider's legitimate domain IS tenant Secrets.
func permissionClaims() []sdkinstall.PermissionClaim {
