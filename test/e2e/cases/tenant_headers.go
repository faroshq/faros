...
// caller to set X-Railgrid-Org; workspace-scoped calls require both
// X-Railgrid-Org and X-Railgrid-Workspace. The headers are not optional and
...
// where the X-Railgrid-Org/X-Railgrid-Workspace headers are missing, point at a
	...
	return features.New("Tenancy/TenantHeaders").
		...
		Assess("mismatched_org_header_rejected", func(ctx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			code, body, err := framework.DoRESTRequest(ctx, http.MethodGet,
				...
				map[string]string{"X-Railgrid-Org": "00000000-0000-0000-0000-000000000000"}, nil)
			if err != nil {
		...
		Assess("mismatched_workspace_header_rejected", func(ctx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			code, body, err := framework.DoRESTRequest(ctx, http.MethodGet,
				workspaceURL(d.hubURL, d.orgUUID, d.wsUUID), d.bearer,
				map[string]string{
					"X-Railgrid-Org":       d.orgUUID,
					"X-Railgrid-Workspace": fakeWS,
				}, nil)
			if err != nil {
