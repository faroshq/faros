	...
	return features.New("Auth/CrossWorkspaceEdgeIsolation").
		Setup(func(ctx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			edgeProxyURLForA := clusterEnv.HubURL +
				...
				"/apis/railgrid.ai/v1alpha1/edges/" + edgeNameA + "/k8s"
			...
			if len(resultB.Kubeconfig) > 0 {
				clientB := framework.NewRailgridClient(framework.RepoRoot(), kcFileB, clusterEnv.HubURL)
				if err := clientB.EdgeCreate(ctx, edgeNameB, "kubernetes", "env=e2e-cross-ws-isolation"); err != nil {
					...
					if clusterNameB != "" {
						edgeProxyURLForB = clusterEnv.HubURL +
							...
							"/apis/railgrid.ai/v1alpha1/edges/" + edgeNameB + "/k8s"
		...
		Teardown(func(ctx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			if data.userBKubeconfig != "" {
				clientB := framework.NewRailgridClient(framework.RepoRoot(), data.userBKubeconfig, "")
