.PHONY: sync-portalkit verify-portalkit verify-ui-conformance
...
verify-ui-conformance: ## Verify provider UI source uses the canonical k-* design vocabulary
...
verify: verify-boilerplate verify-codegen verify-portalkit verify-ui-conformance verify-app-studio-preview-console-dev-key verify-app-studio-eval vet lint build test ## Run all checks
